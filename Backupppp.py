...
DB_POOL_MIN = 1
DB_POOL_MAX = 10  # Reduced pool size
db_pool = None
            ...
            try:
                with conn.cursor() as cur:
                    ...
                    for row in cur.fetchall():
                        matches.append({
                            'match_id': row[0],
                            'match_name': row[1] or f"Match_{row[0]}",
                            'game_mode': row[2] or 'classic',
                            'timestamp': row[3].isoformat() if row[3] else datetime.now().isoformat(),
                            'match_data': row[4] if row[4] else {}
                        })
                    return matches
            finally:
                self.return_connection(conn)
...
async def gameon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update.effective_chat.type == ChatType.PRIVATE:
            await update.message.reply_text(
                "❌ *Please add me to a group to play!*",
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
        ...
        for mode, details in GAME_MODE_DISPLAY.items():
            ...
            mode_text += f"{details['icon']} *{details['title']}*\n"
            mode_text += f"   ↳ {details['desc']}\n\n"
        
        mode_text += "═" * 20 + "\n"
        mode_text += "👆 *Select your preferred game mode*"

        await update.message.reply_text(
            escape_markdown_v2_custom(mode_text),
...
async def view_scorecards(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create keyboard with match list
    keyboard = []
    for match in matches[:10]:  # Show only first 10
    ...
    # Add navigation if more matches exist
    if len(matches) > 10:
        keyboard.append([
...
async def bot_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not check_admin(str(update.effective_user.id)):
        await update.message.reply_text(escape_markdown_v2_custom("❌ Unauthorized"))
        return
    ...
    await update.message.reply_text(escape_markdown_v2_custom(stats), parse_mode=ParseMode.MARKDOWN_V2)
